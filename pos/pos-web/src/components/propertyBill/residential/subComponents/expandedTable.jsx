...
const ExpandedTable = ({ record }) => {
  const tableData = useMemo(() => {
    ...
    const unitsData =
      units && agreementId
        ? units.map((item) => ({ name: item.name, type: "Unit", amount: item.rent }))
        : [];
    const parkingsData =
    ...
    const otherData = [];
    if (!(considerationAmount === null) && (agreementType === 1 || agreementType === 2)) {
    ...
    const data = [...unitsData, ...parkingsData, ...otherData, ...billsArr];
